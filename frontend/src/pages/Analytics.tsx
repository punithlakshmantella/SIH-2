...
function congestionColor(level: string) {
  switch (level) {
    ...
    default:         return 'text-slate-400 bg-slate-950/40 border-slate-800';
  }
}
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 col-span-1">
          ...
          <span className="text-[10px] text-cyan-400 font-mono mt-1 block">Camera detection events</span>
        </div>
      ...
      {s && (
        <div className="flex flex-wrap gap-3 text-xs font-mono">
          ...
          {s.excluded_invalid_records > 0 && (
            <div className="flex items-center space-x-1.5 px-3 py-1.5 rounded-full bg-slate-800 border border-slate-700 text-slate-400">
              ...
              {data.vehicle_type_distribution.map((d: any, i: number) => (
                <div key={i} className="space-y-1">
                  <div className="flex justify-between text-xs font-mono">
                    ...
                    <span className="font-bold text-slate-100">{d.value.toLocaleString()} <span className="text-slate-500 font-normal">({d.pct}%)</span></span>
                  </div>
          ...
          <div className="space-y-3">
            {data?.direction_distribution?.map((d: any, idx: number) => (
              <div key={idx} className="space-y-1">
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-slate-300">{d.direction} <span className="text-slate-600">({d.code})</span></span>
                  <span className="font-bold text-slate-100">{d.count.toLocaleString()} <span className="text-slate-500 font-normal">({d.pct}%)</span></span>
                </div>
                <div className="w-full h-2 rounded-full bg-slate-950 overflow-hidden">
                  <div
              ...
              <tbody className="divide-y divide-slate-800/50">
                {data.congestion_corridors.slice(0, 8).map((c: any, i: number) => (
                  <tr key={i} className="hover:bg-slate-800/30 transition">
                    <td className="py-2 pr-4 text-slate-200 font-medium truncate max-w-[180px]">{c.road_name}</td>
                    <td className="py-2 pr-4 text-slate-400">{c.zone_name}</td>
                    <td className="py-2 pr-4 text-right text-cyan-400">
                      {c.avg_speed_kmh != null ? `${c.avg_speed_kmh} km/h` : 'N/A'}
                    </td>
                    <td className="py-2 pr-4 text-right text-slate-300">{c.volume_label}</td>
            ...
            {odFlows.map((f: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-slate-950/60 border border-slate-800 hover:border-slate-700 transition">
                <div className="flex items-center space-x-2 text-xs font-mono">
                  ...
                  <span className="text-slate-300 font-medium">{f.flow_label || `${f.origin_zone} → ${f.dest_zone}`}</span>
                ...
                <div className="text-right font-mono text-xs">
                  <span className="font-bold text-cyan-400">{f.vehicle_count.toLocaleString()}</span>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-4">
        <h2 className="text-sm font-bold text-slate-100 flex items-center space-x-2">
          ...
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-800 text-slate-400 border border-slate-700 font-mono">Auto-generated from analytics</span>
        </h2>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link to="/cameras" className="text-xs text-cyan-400 hover:text-cyan-300 flex items-center space-x-1 transition">
                ...
                {data.top_cameras.map((c: any, i: number) => (
                  <tr key={i} className="hover:bg-slate-800/30 transition">
                    ...
                    <td className="py-2 pr-4">
                      <Link to={`/cameras/${c.camera_id}`} className="text-cyan-400 hover:text-cyan-300 font-bold transition">
                        {c.camera_id}
                      </Link>
                    </td>
                    <td className="py-2 pr-4 text-slate-300 truncate max-w-[200px]">{c.camera_name}</td>
                    <td className="py-2 pr-4 text-slate-400">{c.zone_name}</td>
      ...
      <div className="flex justify-end">
        <Link
          to="/reports"
          className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-bold transition shadow-md"
        >
