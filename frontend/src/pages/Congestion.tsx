    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800">
          ...
          <span className="text-[10px] text-slate-400 font-mono mt-1 truncate block">
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800">
          ...
          <span className={`text-2xl font-black font-mono mt-1 block ${summary?.critical_risk_count > 0 ? 'text-rose-400' : 'text-slate-300'}`}>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800 col-span-2 lg:col-span-1">
          ...
          <span className="text-[10px] text-cyan-400 font-mono mt-1 block">Peak detection volume</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Interactive Map */}
        <div className="lg:col-span-7 rounded-2xl overflow-hidden border border-slate-800 relative z-0 shadow-lg min-h-[420px]">
          <div ref={mapContainerRef} className="w-full h-full min-h-[420px]" />

          {/* Map Legend */}
          <div className="absolute bottom-4 left-4 bg-slate-900/95 backdrop-blur-md p-3.5 rounded-xl border border-slate-800 text-[11px] space-y-1.5 z-[1000] shadow-lg">
            <span className="font-bold text-slate-300 block text-[10px] uppercase font-mono tracking-wider">Estimated Traffic Density</span>
            ...
            <div className="mt-1 pt-1.5 border-t border-slate-800 text-slate-500 text-[9px]">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-slate-800/60 text-[10px] font-mono">
                      <div>
                        ...
                        <span className="font-bold text-slate-100">{c.estimated_occupancy_pct}%</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="font-bold text-cyan-400">{c.volume_per_hour}/hr</span>
                      </div>
                    </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/70 border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link
            to="/traffic-flow"
            className="flex items-center space-x-1.5 text-xs text-cyan-400 hover:text-cyan-300 font-mono transition"
          >
        ...
        {bottlenecks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {bottlenecks.map((b: any, i: number) => (
              <div key={i} className="p-4 rounded-xl bg-slate-950/70 border border-rose-900/40 space-y-2.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-200 truncate max-w-[180px]">{b.road_name}</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono border ${severityBadge(b.severity)}`}>
                    {b.severity}
                  </span>
                </div>
                <div className="text-[11px] text-slate-400 font-mono">{b.zone_name}</div>
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-slate-400 pt-1 border-t border-slate-900">
                  ...
                  <div>Confidence: <strong className="text-slate-200">{b.confidence_pct}%</strong></div>
                </div>
              </div>
            ))}
          </div>
        ...
        {prediction && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-2">
            ...
            <div className="lg:col-span-7 space-y-3">
              ...
              <div className="h-64 w-full p-4 bg-slate-950/80 rounded-2xl border border-slate-800">
              ...
              <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800 text-[10px] font-mono grid grid-cols-2 sm:grid-cols-4 gap-2">
                <div>
                  <span className="text-slate-500 block">Current Speed</span>
                  <span className="font-bold text-slate-200">{prediction.current_speed_kmh} km/h</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Design Baseline</span>
                  <span className="font-bold text-slate-400">{prediction.baseline_speed_kmh} km/h</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="font-bold text-cyan-400">{prediction.volume_per_hour} veh/hr</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                ...
                <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2.5">
                  <div className="flex items-center justify-between text-xs font-mono">
                    ...
                    <span className="font-bold text-slate-200">{prediction.current_density_pct}% ({prediction.current_risk})</span>
                  </div>
                  <div className="flex items-center justify-between text-xs font-mono">
                    <span className="text-slate-400">Forecast (+{prediction.horizon_minutes}m):</span>
                    <span className="font-bold text-indigo-400 text-sm">{prediction.predicted_density_pct}%</span>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t border-slate-800 text-xs font-mono">
                    <span className="text-slate-400">Forecasted Risk:</span>
              ...
              {/* Disclaimer */}
              <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-800 text-[9px] text-slate-500 font-mono leading-tight">
