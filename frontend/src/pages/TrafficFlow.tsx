...
export default function TrafficFlow() {
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto pb-12">
      ...
      {/* HEADER & FILTERS */}
      <div className="flex flex-wrap items-start justify-between gap-3 bg-slate-900/80 p-4 rounded-2xl border border-slate-800">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* View mode */}
          <div className="flex items-center rounded-xl border border-slate-800 overflow-hidden text-xs font-mono">
            <button
              onClick={() => setViewMode('camera')}
              className={`px-3 py-1.5 transition ${viewMode === 'camera' ? 'bg-cyan-900 text-cyan-300' : 'bg-slate-950 text-slate-400 hover:bg-slate-800'}`}
            >Camera</button>
            <button
              onClick={() => setViewMode('zone')}
              className={`px-3 py-1.5 transition ${viewMode === 'zone' ? 'bg-cyan-900 text-cyan-300' : 'bg-slate-950 text-slate-400 hover:bg-slate-800'}`}
            >Zone</button>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="p-4 rounded-2xl bg-slate-900/70 border border-slate-800">
          ...
          <span className={`text-2xl font-black font-mono mt-1 block ${s?.route_anomalies > 0 ? 'text-rose-400' : 'text-slate-300'}`}>
      ...
      {/* TOP CORRIDOR LABEL */}
      {s?.top_corridor && (
        <div className="flex items-center space-x-2 text-xs font-mono px-4 py-2 bg-slate-900/50 border border-slate-800 rounded-xl">
          <Zap className="w-3.5 h-3.5 text-amber-400" />
          <span className="text-slate-400">Top OD Corridor:</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* MAP */}
        <div className="lg:col-span-7 rounded-2xl overflow-hidden border border-slate-800 relative z-0 shadow-lg" style={{ minHeight: '420px' }}>
          <div ref={mapContainerRef} className="w-full h-full" style={{ minHeight: '420px' }} />

          {/* Legend */}
          <div className="absolute bottom-4 left-4 bg-slate-900/95 backdrop-blur-md p-3 rounded-xl border border-slate-800 text-[11px] space-y-1.5 z-[1000] shadow-lg">
            ...
            <div className="mt-1 pt-1 border-t border-slate-800 text-slate-500 text-[9px]">
        ...
        {/* SIDE PANEL */}
        <div className="lg:col-span-5 bg-slate-900/70 border border-slate-800 rounded-2xl flex flex-col overflow-hidden">
          {/* Tabs */}
            ...
            {(['corridors', 'matrix', 'anomalies'] as const).map(tab => (
              <button
                ...
                className={`flex-1 py-2.5 capitalize transition ${
                  activeTab === tab ? 'bg-slate-800 text-cyan-400 font-bold' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
                }`}
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <div className="text-[10px] font-bold text-cyan-400 font-mono uppercase tracking-wide">Origin</div>
                        <div className="text-xs font-bold text-slate-200 font-mono">{f.origin_camera_id}</div>
                        <div className="text-[10px] text-slate-400 truncate">{f.origin_camera_name}</div>
                        <div className="flex items-center justify-center py-1">
                          <ArrowRight className="w-4 h-4 text-slate-600" />
                        </div>
                        <div className="text-[10px] font-bold text-indigo-400 font-mono uppercase tracking-wide">Destination</div>
                        <div className="text-xs font-bold text-slate-200 font-mono">{f.dest_camera_id}</div>
                        <div className="text-[10px] text-slate-400 truncate">{f.dest_camera_name}</div>
                      </div>

                      {/* Zone transition */}
                      <div className="mt-2 pt-2 border-t border-slate-800/60 text-[10px] font-mono text-slate-400">
                        Zone: <span className="text-cyan-400">{f.origin_zone}</span>
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-2 text-[10px] font-mono">
                        <div>
                          ...
                          <span className="font-bold text-slate-100">{f.transition_count}</span>
                      ...
                      <div className="mt-2 text-[10px] font-mono text-slate-500">
                        Throughput: <span className="text-slate-300">{f.throughput_per_hour}/hr</span>
                        <span className="mx-2">•</span>
                        Distance: <span className="text-slate-300">{f.dist_km} km</span>
                      </div>
                    </div>
                  );
                }) : (
                ...
                {zoneLabels.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="text-[10px] font-mono border-collapse w-full">
                      <thead>
                        <tr>
                          <th className="p-1.5 text-slate-500 text-right text-[9px] font-mono border border-slate-800 bg-slate-950">
                            ↓ Origin / Dest →
                          </th>
                          {zoneLabels.map((z: any) => (
                            <th key={z.id} className="p-1.5 text-center text-[9px] font-bold text-cyan-400 border border-slate-800 bg-slate-950 min-w-[80px] truncate">
                              {z.name.split(' ')[0]}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {zoneLabels.map((originZ: any) => (
                          <tr key={originZ.id}>
                            <td className="p-1.5 text-[9px] font-bold text-indigo-400 border border-slate-800 bg-slate-950 whitespace-nowrap">
                            ...
                            {zoneLabels.map((destZ: any) => {
                              ...
                              return (
                                <td
                                  ...
                                  className={`p-1.5 text-center border border-slate-800 transition cursor-pointer ${
                                    isSame ? 'bg-slate-900 text-slate-700 cursor-default' :
                                    isSelected ? 'bg-cyan-900/60 border-cyan-600 text-cyan-300 font-bold' :
                                    count > 0
                                      ? 'hover:border-cyan-700 text-slate-100 font-bold'
                                      : 'text-slate-700'
                                  }`}
                ...
                {matrixCell && (
                  <div className="p-3 rounded-xl border border-cyan-800 bg-cyan-950/30 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
                      <div>
                        <span className="text-slate-500 block">Transitions</span>
                        <span className="font-bold text-slate-100">{matrixCell.count}</span>
                      </div>
                      {matrixCell.detail && (
                        <>
                          <div>
                ...
                {anomalies.length > 0 ? anomalies.map((a: any, i: number) => (
                  <div key={i} className="p-3 rounded-xl border border-rose-800/60 bg-rose-950/30 space-y-2">
                    ...
                    <div className="text-xs font-mono space-y-1">
                      <div className="text-cyan-400 font-bold">{a.origin_camera_id} <span className="text-slate-500">{a.origin_camera_name}</span></div>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-[10px] font-mono">
                      <div><span className="text-slate-500 block">Distance</span><span className="text-slate-200">{a.dist_km} km</span></div>
                      <div><span className="text-slate-500 block">Elapsed</span><span className="text-slate-200">{a.elapsed_sec}s</span></div>
        ...
        {zoneFlows.length > 0 ? (
          <div className="space-y-2">
            {zoneFlows.slice(0, 8).map((f: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-slate-950/60 border border-slate-800 hover:border-slate-700 transition">
                <div className="flex items-center space-x-3 text-xs font-mono">
                  <span className="text-slate-500 font-bold">{i + 1}.</span>
                  <span className="text-cyan-400 font-bold">{f.origin_zone}</span>
                ...
                <div className="text-right font-mono text-xs space-x-3">
                  <span className="font-bold text-slate-100">{f.transition_count.toLocaleString()}</span>
